...
import { AuthContext, PLAN_FEATURES } from '../contexts/AuthContext';
        ...
        <div style={{ position: 'absolute', left: '16px', top: '16px', bottom: '80px', width: '300px', background: 'rgba(10,10,20,0.97)', border: '1px solid rgba(102,126,234,0.3)', borderRadius: '20px', display: 'flex', flexDirection: 'column', zIndex: 30, backdropFilter: 'blur(20px)', boxShadow: '0 24px 80px rgba(0,0,0,0.7)', overflow: 'hidden' }}>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '12px', gap: '10px', overflow: 'hidden' }}>
                    ...
                    <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {notes.length === 0 ? <p style={{ color: '#475569', textAlign: 'center', fontSize: '13px', marginTop: '20px' }}>No notes yet.</p>
                            : notes.map((n, i) => (
                                <div key={i} style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '10px', padding: '10px 12px', border: '1px solid rgba(255,255,255,0.08)', position: 'relative' }}>
                                    <div style={{ color: '#64748b', fontSize: '11px', marginBottom: '4px' }}>🕐 {n.time}</div>
                                    <div style={{ color: 'white', fontSize: '13px', lineHeight: 1.5, paddingRight: '24px' }}>{n.text}</div>
                                    <button onClick={() => setNotes(prev => prev.filter((_, idx) => idx !== i))} style={{ position: 'absolute', top: '8px', right: '8px', background: 'rgba(239,68,68,0.15)', border: 'none', color: '#f87171', borderRadius: '6px', width: '20px', height: '20px', cursor: 'pointer', fontSize: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>✕</button>
                                </div>
                            ))}
                    </div>
                    {notes.length > 0 && (
                        <button onClick={() => {
                            const content = `CONNECTX MEETING NOTES\n${'='.repeat(40)}\nMeeting Code: ${meetingCode}\nDate: ${new Date().toLocaleDateString('en-IN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}\nTime: ${new Date().toLocaleTimeString()}\n${'='.repeat(40)}\n\n${notes.map((n, i) => `[${n.time}] Note ${i + 1}:\n${n.text}`).join('\n\n')}\n\n${'='.repeat(40)}\nGenerated by ConnectX AI Meeting Platform`;
                            const blob = new Blob([content], { type: 'text/plain' });
                            const url = URL.createObjectURL(blob);
                            const a = document.createElement('a');
                            a.href = url; a.download = `ConnectX-Notes-${meetingCode}-${new Date().toLocaleDateString('en-IN').replace(/\//g, '-')}.txt`;
                            document.body.appendChild(a); a.click();
                            document.body.removeChild(a); URL.revokeObjectURL(url);
                        }} style={{ background: 'linear-gradient(135deg,#667eea,#764ba2)', border: 'none', color: 'white', borderRadius: '10px', padding: '10px', cursor: 'pointer', fontSize: '13px', fontWeight: '700', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', flexShrink: 0 }}>
                            ⬇️ Download Notes ({notes.length})
                        </button>
                    )}
                </div>
            )}
            {tab === 'summary' && (
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '12px', gap: '10px', overflow: 'hidden' }}>
                    ...
                    {summary && (
                        <button onClick={() => {
                            const content = `CONNECTX MEETING SUMMARY\n${'='.repeat(40)}\nMeeting Code: ${meetingCode}\nDate: ${new Date().toLocaleDateString('en-IN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}\nGenerated at: ${new Date().toLocaleTimeString()}\n${'='.repeat(40)}\n\n${summary}\n\n${'='.repeat(40)}\nGenerated by ConnectX AI Meeting Platform`;
                            const blob = new Blob([content], { type: 'text/plain' });
                            const url = URL.createObjectURL(blob);
                            const a = document.createElement('a');
                            a.href = url; a.download = `ConnectX-Summary-${meetingCode}-${new Date().toLocaleDateString('en-IN').replace(/\//g, '-')}.txt`;
                            document.body.appendChild(a); a.click();
                            document.body.removeChild(a); URL.revokeObjectURL(url);
                        }} style={{ background: 'linear-gradient(135deg,#06b6d4,#667eea)', border: 'none', color: 'white', borderRadius: '10px', padding: '10px', cursor: 'pointer', fontSize: '13px', fontWeight: '700', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', flexShrink: 0 }}>
                            ⬇️ Download Summary
                        </button>
                    )}
                </div>
...
export default function VideoMeetComponent() {
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            const elapsed = Math.floor((Date.now() - meetingStartTime.current) / 1000);
            const h = Math.floor(elapsed / 3600);
            const m = Math.floor((elapsed % 3600) / 60);
            const s = elapsed % 60;
            setMeetingDuration(h > 0
    ...
    const connectToSocketServer = () => {
        ...
        socketRef.current.on('connect', () => {
            ...
            socketRef.current.on('user-joined', (id, clients) => {
                ...
                const maxAllowed = PLAN_FEATURES[userPlan]?.maxParticipants ?? 10;
                if (clients.length > maxAllowed && id !== socketIdRef.current) {
                    // Host sees upgrade prompt if room exceeds their plan limit
                    if (isHost) setUpgradePromptFeature('maxParticipants');
                    return;
                }
        ...
        <div className={styles.meetVideoContainer}>
            ...
            {showScorePanel && (
                <div style={{ position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%,-50%)', width: '360px', maxHeight: '80vh', background: 'rgba(10,10,20,0.98)', border: '1px solid rgba(102,126,234,0.4)', borderRadius: '24px', display: 'flex', flexDirection: 'column', zIndex: 40, backdropFilter: 'blur(20px)', boxShadow: '0 32px 100px rgba(0,0,0,0.8)', overflow: 'hidden' }}>
                    ...
                    {scoreData && !scoreLoading && !scoreData.error && (
                        <div style={{ padding: '12px 16px', borderTop: '1px solid rgba(255,255,255,0.07)', flexShrink: 0 }}>
                            <button onClick={() => {
                                const metrics = Object.entries(scoreData.metrics || {}).map(([k, v]) => `  ${k.charAt(0).toUpperCase() + k.slice(1)}: ${v}/100`).join('\n');
                                const highlights = (scoreData.highlights || []).map(h => `  • ${h}`).join('\n');
                                const improvements = (scoreData.improvements || []).map(h => `  • ${h}`).join('\n');
                                const participants = (scoreData.participantScores || []).map(p => `  ${p.name} — ${p.score}/100 (${p.role})`).join('\n');
                                const content = `CONNECTX AI MEETING SCORE REPORT\n${'='.repeat(40)}\nMeeting Code: ${meetingCode}\nDate: ${new Date().toLocaleDateString('en-IN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}\nDuration: ${meetingDuration}\nParticipants: ${participants || 'N/A'}\n${'='.repeat(40)}\n\nOVERALL SCORE: ${scoreData.overallScore}/100  Grade: ${scoreData.grade}\n\nSUMMARY:\n${scoreData.summary}\n\nMETRICS:\n${metrics}\n\nHIGHLIGHTS:\n${highlights}\n\nIMPROVEMENTS SUGGESTED:\n${improvements}\n\nPARTICIPANT SCORES:\n${participants}\n\n${'='.repeat(40)}\nGenerated by ConnectX AI Meeting Platform`;
                                const blob = new Blob([content], { type: 'text/plain' });
                                const url = URL.createObjectURL(blob);
                                const a = document.createElement('a');
                                a.href = url; a.download = `ConnectX-Score-${meetingCode}-${new Date().toLocaleDateString('en-IN').replace(/\//g, '-')}.txt`;
                                document.body.appendChild(a); a.click();
                                document.body.removeChild(a); URL.revokeObjectURL(url);
                            }} style={{ width: '100%', background: 'linear-gradient(135deg,#06b6d4,#667eea)', border: 'none', color: 'white', borderRadius: '10px', padding: '10px', cursor: 'pointer', fontSize: '13px', fontWeight: '700', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
