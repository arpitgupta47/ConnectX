...
function AIAssistant({ isOpen, onClose, username, meetingCode, participants }) {
    ...
    // FIX: Call backend proxy instead of Anthropic directly
...
export default function VideoMeetComponent() {
    ...
    // FIX: videos stores ONLY remote participants — self is always shown via localVideoref
    ...
    // FIX: Jab lobby se meeting screen pe jaate hain, localVideoref naya DOM element hota hai
    // is useEffect se stream wapas assign ho jaati hai
    useEffect(() => {
    ...
    const getUserMediaSuccess = (stream) => {
        ...
        for (let id in connections) {
            ...
            connections[id].createOffer().then(d => connections[id].setLocalDescription(d).then(() =>
    ...
    const connectToSocketServer = () => {
        ...
        socketRef.current.on('connect', () => {
            ...
            socketRef.current.on('user-joined', (id, clients) => {
                // FIX: If I just joined, mark host status; else play join sound for others
                ...
                // FIX: Only create connections for OTHER participants, never self
                clients.forEach(socketListId => {
                    ...
                    // FIX: ontrack — never add self stream as remote video
                    connections[socketListId].ontrack = (event) => {
                        ...
                        setVideos(vids => {
                            ...
                            const alreadyExists = vids.find(v => v.socketId === socketListId);
                            if (alreadyExists) {
                                const updated = vids.map(v => v.socketId === socketListId ? {...v, stream: remoteStream} : v);
                                videoRef.current = updated;
                                return updated;
                            }
                            const newList = [...vids, {socketId: socketListId, stream: remoteStream}];
                            videoRef.current = newList;
                            return newList;
                        });
                    };

                    // Add local tracks to this peer connection
                    const streamToSend = window.localStream || (() => {
                        const bs = new MediaStream([black(), silence()]);
                        window.localStream = bs;
                        return bs;
                    })();
                    streamToSend.getTracks().forEach(t => connections[socketListId].addTrack(t, streamToSend));
                });

                // FIX: If I am the newly joined user, send offers to everyone already in room
                if (id === socketIdRef.current) {
                    clients.forEach(id2 => {
                        if (id2 === socketIdRef.current) return;
                        // addTrack is already done above; now create offer
    ...
    const handleVideo = () => { const t=window.localStream?.getVideoTracks()[0]; if(!t)return; t.enabled=!t.enabled; setVideo(t.enabled); };
    const handleAudio = () => { const t=window.localStream?.getAudioTracks()[0]; if(!t)return; t.enabled=!t.enabled; setAudio(t.enabled); };
    ...
    const connect = () => {
        ...
        // Stream is already in window.localStream from getPermissions
        // After askForUsername=false, meeting screen mounts with new localVideoref
        // useEffect will reassign stream to new ref
        connectToSocketServer();
    };

    // FIX: total = self + remote videos (no double counting)
    ...
    return (
        <div className={styles.meetVideoContainer}>
            ...
            {isHost && admitRequests.length > 0 && (
                <div style={{position:'absolute',top:'16px',right:'16px',zIndex:30,display:'flex',flexDirection:'column',gap:'10px'}}>
                    {admitRequests.map(req=>(
                        <div key={req.socketId} style={{background:'rgba(15,15,26,0.97)',border:'1px solid rgba(102,126,234,0.4)',borderRadius:'14px',padding:'14px 18px',display:'flex',alignItems:'center',gap:'12px',backdropFilter:'blur(16px)',boxShadow:'0 8px 32px rgba(0,0,0,0.6)',animation:'slideIn 0.3s ease',maxWidth:'320px'}}>
            ...
            <div className={`${styles.conferenceView} ${getGridClass(totalParticipants)}`}>
                ...
                {/* Remote tiles — FIX: each unique socketId renders once */}
                {videos.map(v => (
            ...
            {/* REACTION PICKER */}
            {showReactionPicker && (
                <div style={{position:'absolute',bottom:'80px',left:'50%',transform:'translateX(-50%)',zIndex:40,background:'rgba(15,15,26,0.97)',border:'1px solid rgba(102,126,234,0.3)',borderRadius:'16px',padding:'12px 16px',display:'flex',gap:'8px',backdropFilter:'blur(20px)',boxShadow:'0 16px 48px rgba(0,0,0,0.6)',animation:'fadeUp 0.2s ease'}}>
                    {['👍','❤️','😂','😮','🎉','👏','🔥','💯'].map(emoji=>(
